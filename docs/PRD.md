...
## Delivery / acceptance
Working dev and production builds; typecheck; browser verification for opening, scratch/reveal, carousel, postcards, responsive layout and reduced motion. Document actual test results and remaining integration/setup work. Email integration is the only planned functional deferral. No RSVP, guest tracking, payment, autoplay audio, fabricated venue, or false sending success. No deployment without configured hosting; provide reproducible run/build steps.

## Asset / integration follow-up
Postcard artwork is a coordinated collection of 11 custom generated destination illustrations. Confirm bride spelling and Davao venue if desired. Configure destination email and secure server endpoint later (validation, rate limiting, email provider); no secrets in browser code. Supplied couple illustration belongs next to the countdown. Include the original teacup illustration beneath the celebrations. No em dashes in site copy.
