...
export default function PhotoCarousel(){
 const [index,setIndex]=useState(0);const [playing,setPlaying]=useState(false);const [hover,setHover]=useState(false);const touch=useRef<number|null>(null);
 ...
 <div className="moments-copy"><span className="eyebrow">02 / A few pages from our story</span><h2>All the miles.<br/>All the <em>moments.</em></h2><p>Between the flights and the farewells, we made a life full of little things worth keeping.</p><div className="carousel-controls"><button aria-label="Previous photograph" onClick={()=>move(-1)}>←</button><span aria-live="polite" aria-atomic="true">{String(index+1).padStart(2,'0')} <i>/ 03</i></span><button aria-label="Next photograph" onClick={()=>move(1)}>→</button><button className="play-control" onClick={()=>setPlaying(p=>!p)} aria-pressed={playing} aria-label={playing?'Pause slideshow':'Play slideshow'}>{playing?'Pause':'Play'}</button></div></div>
 <div className="photo-stack" onTouchStart={e=>touch.current=e.touches[0].clientX} onTouchEnd={e=>{if(touch.current!==null){const d=e.changedTouches[0].clientX-touch.current;if(Math.abs(d)>40)move(d>0?-1:1);touch.current=null}}}><div className="stack-back"/><div className="stack-back second"/><div className="photo-window">{photos.map((p,i)=><img key={p.src} className={i===index?'active':''} src={`/images/${p.src}.webp`} alt={p.alt} aria-hidden={i!==index} aria-current={i===index ? 'true' : undefined} width="1000" height="1100" loading="lazy"/>)}</div><p className="photo-caption" aria-live="polite" aria-atomic="true">{photos[index].caption}</p><span className="photo-corner" aria-hidden="true">R & G</span></div>
 </section>
}
